...
var lang_names_list = ['ar', 'cs', 'de', 'el', 'en', 'es', 'fr', 'it', 'nl', 'pl', 'pt', 'ru', 'tr', 'zh-CN', 'zh-TW'];
...
setTimeout(function() {
  runOnceAvailable('a[trdl]', function() {
    var trcl_list = document.querySelectorAll('a[trdl]');
    for (var i = 0; i < trcl_list.length; ++i) {
      var trcl = trcl_list[i];
      (function (trcl) {
        var url = trcl.getAttribute('href');
        ...
        if (is_local) {
          trcl.addEventListener('click', function(evt) {
            addLog('Download', {'Url': trcl.getAttribute('href'), 'Ver': trcl.getAttribute('trdl')});
          });
        } else {
          trcl.addEventListener('click', function(evt) {
            addLog('Download', {'Url': trcl.getAttribute('href'), 'Ver': trcl.getAttribute('trdl')}, function() {
              window.location = trcl.getAttribute('href');
            }, 500);
            evt.preventDefault();
          });
